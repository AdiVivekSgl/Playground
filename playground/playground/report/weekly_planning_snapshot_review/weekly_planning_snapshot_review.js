...
frappe.query_reports["Weekly Planning Snapshot Review"] = {
	...
	onload(report) {
		report.page.add_inner_button(__("Create Weekly Snapshot (Draft)"), () => {
			frappe.confirm(
				__("This freezes the current open Sales Order demand into a new DRAFT Weekly Planning Snapshot. Review it (adjust Committed Prodn line-wise), then Submit to approve. Continue?"),
				() => {
					frappe.call({
